...
namespace GVC_EXPORTER_PLUGIN.Functions._Binary
{
    ...
    internal class Context
    {
        ...
        public ModelItemCollection _itemsR { get; set; }
        ...
        public Chunks_Oriented.OrientedBoundingBox _OBB_modelBox { get; set; }
        ...
        public void Clear()
        {
            ...
            _itemsR = new ModelItemCollection();
            _points = new List<byte[]>();
            _pointsLoacated = new List<byte[]>();
            _chunks = new List<byte[]>();
            _modelBox = new BoundingBox3D();
            _OBB_modelBox = new Chunks_Oriented.OrientedBoundingBox();
            _RenderFlag = false;
    ...
    public static class OBB_ContextInitializer
    {
        public static void InitializeContext(double x = 2, double y = 2, double z = 1, double rotation = 0.0)
        {
            ...
            try
            {
                ...
                Context.Instance._itemsR = Context.Instance._items;
                ...
                Context.Instance._OBB_modelBox = Chunks_Oriented.RotateOBB_Z_FromBoundingBox(Context.Instance._modelBox, rotation, 0);
                Context.Instance._metadata["state"] = ("done", 0, 0);

                // Gera os chunks e localiza os pontos
                Context.Instance._metadata["state"] = ("Gerando chunks", 0, 0);
                var obbs = Chunks_Oriented.ChunkOBBsFromOBB(Context.Instance._OBB_modelBox, x, y, z);
                Context.Instance._chunks = obbs.Select(Chunks_Oriented.PackOBB).ToList();
                Context.Instance._pointsLoacated = Chunks_Oriented.LocatePointsOBB(obbs);
                Context.Instance._metadata["state"] = ("done", 0, 0);
    ...
    public static class ContextToModel
    {
        ...
        private static Dictionary<int, List<byte[]>> GroupPointsByChunkId()
        {
            return Context.Instance._points
                ...
                {
                    Point = point,
                    ChunkId = PointCloudExporter.UnpackPoint(point).ChunkId
                })
        ...
        private static ModelItemCollection GetModelItemsFromPoints(List<byte[]> packedPoints)
        {
            ...
            {
                items.AddRange(PointCloudExporter.GetModelItemsFromPackedPoint(point));
                count++;
